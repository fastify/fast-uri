...
/**
 * Returns the round-trippable `escapedHost` of a zoned IPv6 host, but only
 * when it is well-formed and normalizes back to the current `component.host`.
 * This validates the value on use, so a caller-supplied `escapedHost` that
 * does not describe the host (or a host reassigned after `parse`) is never
 * trusted and recomposition falls back to the normal path.
 *
 * @param {import('../types/index').URIComponent} component
 * @returns {string|undefined}
 */
function validateEscapedHost (component) {
  const escaped = component.escapedHost
  if (escaped === undefined) return undefined
  const derived = normalizeIPv6('[' + escaped + ']')
  if (derived.isIPV6 === true && derived.host === component.host) {
    return escaped
  }
  return undefined
}
...
function recomposeAuthority (component) {
  ...
  if (component.host !== undefined) {
    ...
    if (!isIPv4(host)) {
      // A parse-time escaped form is authoritative only while it still
      ...
      // validated round-trippable form keeps parse, normalize, serialize and
      // equal in agreement.
      const escapedHost = validateEscapedHost(component)
      if (escapedHost !== undefined) {
        host = `[${escapedHost}]`
      } else {
