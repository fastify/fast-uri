...
function recomposeAuthority (component) {
  ...
  if (component.host !== undefined) {
    ...
    if (!isIPv4(host)) {
      // A parse-time escaped form is authoritative. Re-deriving an IPv6 zone
      // from the single-"%" component form is ambiguous when the zone begins
      // with "25": it is misread as a %25 separator, rewriting e.g. zone
      // "25eth0" into "eth0" (or destroying a zone of just "25"). Carrying
      // the round-trippable form keeps parse, normalize, serialize and equal
      // in agreement.
      if (component.escapedHost !== undefined) {
        host = `[${component.escapedHost}]`
      } else {
