...
/**
 * Private, non-enumerable marker holding the round-trippable %25-escaped form
 * of a zoned IPv6 host together with the exact `host` value it was derived
 * from. It is stored under a Symbol so only `parse` can set it and so that a
 * user-supplied `escapedHost` string is never trusted by `serialize`.
 */
const ZONED_IPV6_HOST = Symbol('fast-uri zoned IPv6 host')

/**
 * Stores the round-trippable %25-escaped form of a zoned IPv6 host alongside
 * the host value it was derived from. Recomposition only trusts it while
 * `component.host` is unchanged.
 *
 * @param {import('../types/index').URIComponent} component
 * @param {string} host
 * @param {string} escapedHost
 */
function setZonedIPv6Host (component, host, escapedHost) {
  Object.defineProperty(component, ZONED_IPV6_HOST, {
    value: { host, escapedHost },
    enumerable: false,
    writable: false,
    configurable: true
  })
...
/**
 * Returns the %25-escaped host for `component.host`, but only when it still
 * matches the host value the form was derived from (i.e. the host was not
 * reassigned after parse). Returns `undefined` when there is no zoned IPv6
 * host or when the host has since been mutated.
 *
 * @param {import('../types/index').URIComponent} component
 * @returns {string|undefined}
 */
function getZonedIPv6Host (component) {
  const stored = component[ZONED_IPV6_HOST]
  if (stored !== undefined && component.host === stored.host) {
    return stored.escapedHost
  }
  return undefined
}

/**
 * Copies the zoned IPv6 host marker from `source` to `target`, if present.
 *
 * @param {import('../types/index').URIComponent} target
 * @param {import('../types/index').URIComponent} source
 */
function copyZonedIPv6Host (target, source) {
  const stored = source[ZONED_IPV6_HOST]
  if (stored !== undefined) {
    setZonedIPv6Host(target, stored.host, stored.escapedHost)
  }
}
...
function recomposeAuthority (component) {
  ...
  if (component.host !== undefined) {
    ...
    if (!isIPv4(host)) {
      // A parse-time escaped form is authoritative but only while the host
      // is unchanged. Re-deriving an IPv6 zone from the single-"%" component
      // form is ambiguous when the zone begins with "25": it is misread as a
      // %25 separator, rewriting e.g. zone "25eth0" into "eth0" (or
      // destroying a zone of just "25"). Carrying the round-trippable form
      // keeps parse, normalize, serialize and equal in agreement, while a
      // host reassigned after parse falls through to the normal path.
      const authoritativeEscapedHost = getZonedIPv6Host(component)
      if (authoritativeEscapedHost !== undefined) {
        host = `[${authoritativeEscapedHost}]`
      } else {
